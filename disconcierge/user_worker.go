...
func (u *userCommandWorker) User() User {
	u.userMu.Lock()
	defer u.userMu.Unlock()
	return *u.user
}

func (u *userCommandWorker) SetUser(user *User) {
	u.userMu.Lock()
	defer u.userMu.Unlock()
	u.user = user
}
...
func (u *userCommandWorker) getRunSteps(
	...
	if len(runSteps) > 0 {
		affected, insErr := u.dc.writeDB.Create(context.TODO(), &runSteps)
		if insErr != nil {
			logger.Error("error saving run steps", tint.Err(err))
			return errors.Join(err, insErr)
		}
		logger.Info("saved run steps", "affected", affected, "chat_command", req)
	}
	return nil
}
...
func (u *userCommandWorker) handleClearCommand(
	...
	ctx, cancel = context.WithTimeout(ctx, clearCommandTimeout)
	defer cancel()

	if err := req.execute(ctx, u.dc); err != nil {
...
func (u userCommandWorker) LogValue() slog.Value {
	return slog.GroupValue(slog.Any("user", u.user))
}
...
func (u *userCommandWorker) handleChatCommand(
	...
	if panicked {
		go func() {
			...
			defer notifyCancel()
			discordNotifyCommandPanicked(notifyCtx, log, req, u.dc.discord)
		}()
		return
	}
	...
	case openai.RunStatusCompleted, openai.RunStatusIncomplete, openai.RunStatusFailed, openai.RunStatusCancelled, openai.RunStatusRequiresAction, openai.RunStatusExpired:
		go func() {
			runStepCtx, runStepCancel := context.WithTimeout(parentCtx, time.Minute)
			defer runStepCancel()
			log.InfoContext(runStepCtx, "getting run steps in background for chat command")
			if stepErr := u.getRunSteps(runStepCtx, req); stepErr != nil {
				log.ErrorContext(runStepCtx, "error getting run steps", tint.Err(stepErr))
			}
		}()
	}
...
func (u *userCommandWorker) runChatCommand(ctx context.Context, c *ChatCommand) {
	d := u.dc
		...
		if c.State != ChatCommandStateIgnored {
			go func() {
				...
				updateChatCommandStateAndDeleteInteraction(
					uctx, logger, d.writeDB, c, ChatCommandStateIgnored,
				)
			}()
	...
	if _, err := d.writeDB.Updates(
		...
		defer ecancel()
		_, _ = c.handler.Edit(ectx, &discordgo.WebhookEdit{Content: &config.DiscordErrorMessage})
		return
	}
	c.Answer(ctx, d)
}
...
// updateChatCommandStateAndDeleteInteraction sets the given state on the
// provided ChatCommand, and deletes the discord interaction if it was previously
// acknowledged and its token hasn't expired
func updateChatCommandStateAndDeleteInteraction(
	...
	go func() {
		defer wg.Done()
		if _, err := db.Update(ctx, c, columnChatCommandState, newState); err != nil {
			logger.ErrorContext(ctx, "error updating command state", tint.Err(err))
		}
	}()
